...
?>
<div class="container">
	<h2 id="searchterm"><?php $row=mysqli_fetch_array($result); echo $row['category'];?></h2>
	<div id="cat-videos" class=" tab-pane fade active in video-grid">
					...
					while($row = mysqli_fetch_array($result)){
				        ...
				        <div class="span4 video text-center box"> 
				        	<a href="/video.php?id=<?php echo $row[$i]['id'];?>" class="video-overlay">
				        		<div>
				        			<h2><?php echo $row[$i]['name'];?></h2>
				        			<span><?php echo $row[$i]['category'];?></span>
				        		</div>
				        	</a>    
				       		<img src="videos/thumbnails/<?php echo $row[$i]['image'] ?>" alt="<?php echo $row[$i]['name'] ?>" />
				        </div>
				        <?php if($i===2){
							?></div><?php
							$i=-1;
